...
| `/api/youtube-analytics` | YouTube Analytics API v2 | **OAuth リフレッシュトークン** | 視聴維持率・平均視聴時間・総再生時間・登録者増減・トラフィックソース |
...
`api/_google.js`（認証）と `api/_auth.js`（Basic認証）は共有モジュールです。
...
| タブ | 内容 |
|---|---|
| 概要 | 前期間比つきKPI／伸びた動画トップ3／データから生成した「今やること」 |
| YouTube分析 | 登録者数・視聴回数・登録者純増・平均視聴維持率・総再生時間／前期間比／トラフィックソース／トップ10／検索・長さ絞り込み・ページング対応の動画別テーブル／1,000視聴あたり登録者増加 |
| サイト送客・CV | 期間重複を除いたアクティブユーザー／セッション・PV・キーイベント／**YouTube経由セッション・キーイベント・セッションキーイベント率**／前期間比／参照元別／ページ別 |

期間は 28 / 90 / 180 / 365 日から選択。45日を超える期間は棒グラフを週次合計に丸めます。

## 評価の前提
再生数の増減だけで判断しないこと。必ず `/nensyuagent/` への送客（GA4のセッション・キーイベント）と
接続して評価します。GA4 は StockSun 本体と同一プロパティのため、
API側で `pagePath` が `/nensyuagent` 本体または `/nensyuagent/` 配下のものだけに絞り込んでいます。
